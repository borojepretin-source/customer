...
const getTransporter = () => {
    ...
    if (!host || !user || !pass) {
        ...
        throw new https_1.HttpsError('failed-precondition', 'SMTP service is not configured properly.');
    }
...
exports.sendPhotoEmail = (0, https_1.onCall)({ maxInstances: 10 }, async (request) => {
    ...
    if (!email || !photoUrl || !sessionId) {
        throw new https_1.HttpsError('invalid-argument', 'Parameters email, photo_url, and session_id are required.');
    }
    try {
        ...
        if (photoBuffer.length === 0) {
            throw new https_1.HttpsError('failed-precondition', 'Photo attachment is empty. Email will not be sent without a valid photo.');
        }
        // 2. Siapkan SMTP transporter
        const transporter = getTransporter();
        // 3. Susun isi email sesuai spesifikasi
        const fromAddress = process.env.SMTP_FROM || `"${process.env.SMTP_FROM_NAME || 'Boro Photo Booth'}" <${process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER}>`;
        const textBody = `Halo,
...
<body style="margin:0;padding:0;background-color:#f4f4f7;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f7;padding:40px 0;">
    ...
    `;
        ...
        throw new https_1.HttpsError('internal', `Gagal mengirim email: ${errMsg}`);
    }
});
